 ...
 * filePath:string
 * }]}[]
 * |{index, checkForFilesFoldersConstraints, checkForFilesFolders, processFilesFolders:"WITHIN",
 *   quantifierXPathQuery, constraintXPathQuery}[]
 * }
...
export const checkRulesForFile = (xmlFiles, ruleTable, filePath) => {
    const targetXml = xmlFiles.filter((d) => {
    ...
    for (let i = 0; i < ruleTable.length; i++) {
        ...
        const prevQuantifierResult = ruleResultI.quantifierResult.slice(0);
        const prevSatisfiedResult = ruleResultI.satisfiedResult.slice(0);
        const prevSatisfied = ruleResultI.satisfied;
        const prevViolated = ruleResultI.violated;
...
export const runRulesByTypes = (xmlFiles, ruleI) => {
    ...
    const checkForFilesFolders = ruleI.checkForFilesFolders.slice(0); // deep copy
...
const runXPathQueryWithin = (xmlFile, ruleI) => {
    const quantifierResult = runXPathQuery(xmlFile, ruleI.quantifierXPathQuery[0]);
    const satisfiedResult = runXPathQuery(xmlFile, ruleI.constraintXPathQuery[0]);
    // compare results
    const violatedResult = violatedResults(quantifierResult, satisfiedResult);
    const resultData = {
    ...
    const resultArray = ruleI.xPathQueryResult.filter((d) => {
...
const runXpathQueryBetween = (xmlFiles, ruleI) => {
    ...
    for (let j = 0; j < xmlFiles.length; j++) {
        const quantifierResult = runXPathQuery(xmlFiles[j], ruleI.quantifierXPathQuery[0]);
        const constraintResult = runXPathQuery(xmlFiles[j], ruleI.constraintXPathQuery[0]);
        // compare results
        const violatedResult = violatedResults(quantifierResult, constraintResult);
        const resultData = {
            quantifierResult: quantifierResult,
...
const runXpathQueryMixed = (xmlFiles, ruleI) => {
    ...
    for (let j = 0; j < xmlFiles.length; j++) {
        // compare results
        const violatedResult = containResults(constraintResultArray[j].results, quantifierResultArray[j].results);
        const resultData = {
...
const findAndReturnToBase = (xmlFiles, xpathQueries) => {
    let result1; let result2; const result3 = [];
    for (let j = 0; j < xmlFiles.length; j++) {
...
const runXPathQuery = (xmlFile, xpathQuery) => {
    ...
    const xml = parser.parseFromString(xmlFile.xml, "text/xml");
    if (!xml.evaluate) {
    ...
    // run xpath queries
    const foundNodes = xml.evaluate(xpathQuery, xml, nsResolver, XPathResult.ANY_TYPE, null);
    let node = foundNodes.iterateNext();
    let index = 0;
    while (node) {
        const xmlAndText = getXmlData(xml, xpathQuery, index);
        result.push({
...
const violatedResults = (quantifierResult, satisfiedResult) => {
    const mismatches = [];
    const sliceArr = satisfiedResult.slice(0);
...
const getXmlData = (mainXml, query, index) => {
    ...
    const xml = Utilities.cloneXML(mainXml);
    const xmlCopy = Utilities.cloneXML(mainXml);

    const nodes = xml.evaluate(query, xml, nsResolver, XPathResult.ANY_TYPE, null);
    const nodesCopy = xml.evaluate(query, xmlCopy, nsResolver, XPathResult.ANY_TYPE, null);
    ...
    const surroundingNodes = getSurroundingNodes(resNodes);
    const surroundingNodesText = new XMLSerializer().serializeToString(surroundingNodes)
    ...
    const serializedRes = new XMLSerializer().serializeToString(res);
    const cleanedRes = serializedRes
        .replace(/<annotation[^>]*>.*?<\/annotation>\s*\n?/g, "")
        .replace(/<comment[^>]*>.*?<\/comment>\s*\n?/g, "");

    // Get the first two lines
    const resTextArray = cleanedRes.split(/\r?\n/).filter((line) => line.trim() !== "");
    const resText = resTextArray.length > 1 ? resTextArray[0] + "\n" + resTextArray[1] : resTextArray[0];

    let par = res; let nameIndex; let fileName = "";
    if (res.children) {
...
const isValidXPathQueries = (ruleI) => {
    const parser = new DOMParser();
    const xml = parser.parseFromString("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n" +
...
export const runXpathQueryOnFileGroup = (xmlFiles, fileGroup,
    ...
    let snippets = [];
    const files = {all: [], found: []};
    const xmlFilesToVerify = xmlFiles.filter((d) => d.filePath.startsWith(projectPath + fileGroup));
    files.all = xmlFilesToVerify.map((d) => d.filePath.replace(projectPath, ""));

    for (let j = 0; j < xmlFilesToVerify.length; j++) {
        try {
            const results = runXPathQuery(xmlFilesToVerify[j], xpathQuery);
            if (results.length > 0) {
...
export const runXpathQueryMinedRules = (xmlFiles, projectPath, identifierGroupValue) => {
    const newIdentifierGroupValue = JSON.parse(JSON.stringify(identifierGroupValue));
    ...
    if (!newIdentifierGroupValue.quantifierSnippets) {
        ...
        if (isValidXPathQuery(quantifierXPathQuery)) {
            ...
            for (const fileGroup of newIdentifierGroupValue.filesFolders) {
                const results = runXpathQueryOnFileGroup(xmlFiles, fileGroup, projectPath, quantifierXPathQuery);
                newIdentifierGroupValue.quantifierSnippets
    ...
    const childrenKeys = Object.keys(newIdentifierGroupValue.constraintsXPathQuery);
        ...
        newIdentifierGroupValue.constraintsXPathQuery[key].forEach((cXpath, index) => {
            ...
            if (!newIdentifierGroupValue.constraintsSnippets[key][index]) {
                ...
                if (isValidXPathQuery(constraintXPathQuery)) {
                    ...
                    for (const fileGroup of newIdentifierGroupValue.filesFolders) {
                        const results = runXpathQueryOnFileGroup(xmlFiles, fileGroup, projectPath,
