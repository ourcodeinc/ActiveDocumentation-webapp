        ...
        for (let q = 0; q < constructor.length; q++) {
            ...
            if (constrBodyList !== undefined) {
                ...
                for (let v = 0; v < constructorExpr.length; v++) {
                    ...
                    if (((name !== null) && (name.text === "this")) && ((op !== null)
                        && (op.text === "="))) {
                ...
                if (memVarSet.length > 0) {
                    ...
                    for (let x = 0; x < memVarSet.length; x++) {
                        augmentedMemVarSet[x] = " declaration statement with name \""
                            + memVarSet[x] + "\"";
                    }
                    ...
                    if (!attributeList.has(name)) {
                        ...
                        let command = "src:block/src:constructor[(src:parameter_list/" +
                            "src:parameter/src:decl[src:name[text()=\""
                            + memVarSet[0] + "\"]";
                        for (let x = 1; x < memVarSet.length; x++) {
                            command = command + " and src:block/src:constructor[(src:parameter_list/" +
                                "src:parameter/src:decl[src:name[text()=\""
                                + memVarSet[x] + "\"]";
                        }
                ...
                for (let u = 0; u < allExpr.length; u++) {
                    ...
                    if ((op !== null) && (op.text === "new") && (call !== null)
                        && (call.text !== null)) {
                        name = "class with constructor with expression statement \"new "
                            + call.text + "\"";

                        if (!attributeList.has(name)) {
                            // src:expr[src:call/src:name/text()="constructor_name" and
                            // src:operator[text()=\"new\" and position()=1]]
                            // Old command
                            /*
            ...
            if (allParamTypes !== "") {
                ...
                if (!attributeList.has(name)) {
                    ...
                    // Make query
                    let command = "src:block/src:constructor[(src:parameter_list/" +
                        "src:parameter/src:decl[src:type['"
                        + constrParamTypes[0] + "']";

                    for (let u = 1; u < constrParamTypes.length; u++) {
                        command = command + " and "
                            + "src:block/src:constructor[(src:parameter_list/" +
                            "src:parameter/src:decl[src:type['"
                            + constrParamTypes[u] + "']";
                    }
...
export const findClsFunctions = (subCL, attributeList, id_start, queryMap) => {
    ...
    // Attribute name
    let name = "class with visibility \"" + clsSpecificity + "\"";
    ...
    for (let x = 0; x < funcList.length; x++) {
        ...
        if (modifiesMemberVar !== null) {
            for (let n = 0; n < modifiesMemberVar.length; n++) {
                ...
                if (attrName != null && attrName.text === "this" && op !== null
                    && op.text === "=" && call !== null) {
        ...
        if (fncSpec.length === 0) {
            ...
            for (let n = 1; n < fncSpec.length; n++) {
                let spec = fncSpec[n];
                // If statement here to avoid adding the visibility specifier
                // twice
        ...
        // (2) An attribute containing information  about the function return type, name,
        //     and visibility
        if (fncParams == null) {
            name = "class with function with (name \""
                + fncName.text
            ...
            if (!attributeList.has(name)) {
                ...
                let command = "//src:function[count(src:parameter_list/src:parameter)=0 " +
            ...
            if (allFncParamTypes !== "") {
                ...
                if (!attributeList.has(name)) {
                    ...
                    for (let m = 0; m < fncTypes.length; m++) {
                        command = command + " and src:parameter_list/src:parameter/" +
                            "src:decl[src:type['" + fncTypes[m] + "']]";
                    }
                    // Next we add to it the part of the command having to do with visibility
                    command = command + " and src:specifier[text()=\"" + fncSpecType + "\"]";
                    // Next we add the part about the  return type
    ...
    if (constructor.length > 0) {
        for (let q = 0; q < constructor.length; q++) {
            ...
            if (allParamTypes !== "") {
                ...
                // If the constructor has parameters and all of them were stored
                if (allParamsStored === true) {
                    name = "class with constructor with \"storing all parameters " +
                        "as member variables\"";
                    // Check if this attribute has been seen globally
                    if (allAttributes.has(name)) {
                        attributes.push(allAttributes.get(name));
